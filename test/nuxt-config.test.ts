...
afterEach(() => {
  vi.unstubAllGlobals();
  delete process.env.PLUTO_SUPABASE_BLOG_PATH;
  vi.resetModules();
});

describe("supabase-blog layer resolution", () => {
  it("defaults to the published npm package when PLUTO_SUPABASE_BLOG_PATH is unset", async () => {
    delete process.env.PLUTO_SUPABASE_BLOG_PATH;

    const { default: config } = await import("../nuxt.config");

    expect(config.extends).toEqual([
      ["@plutocms/supabase-blog", { install: true }],
    ]);
  });

  it("honors PLUTO_SUPABASE_BLOG_PATH when set", async () => {
    process.env.PLUTO_SUPABASE_BLOG_PATH = "../supabase-blog";

    const { default: config } = await import("../nuxt.config");

    expect(config.extends).toEqual([
      ["../supabase-blog", { install: true }],
    ]);
  });
});
